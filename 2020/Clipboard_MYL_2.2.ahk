...
CapsLock & h:: msgbox,64,Clipboard v2.0 for WS1,This Clipboard v2.0 is scripted by MYL :) `nIt allows you to copy 10 texts simultaneously.`nYou can set predefined texts in the slots in this script and pressing CapsLock + (+) will store those predefined texts in their dedicated slots`n`nFollowing are the shortcuts included in this script:`nCtrl + (0 - 9): To copy a text and store it in its dedicated slot. `n(Ctrl+1 to store in slot 1, Ctrl+2 to store in slot 2, so on...)`n`nAlt + (0 - 9): To paste a string from its slot.`n`nCapsLock + (-): It clears all slots and removes all copied texts.`n`nCapsLock + S: Shows current values of all slots as well as Windows' default clipboard.`n`n`nThank You for trying this app. You can reach me at fb.com/mylakhana for feedbacks and recommendations :)`nHave fun!`n`nWhat's new in  2.1:`nBecause of this script, you weren't able to use Ctrl + (1-9) shortcuts in browsers to switch to specified tabs. So this script has been updated and shortcuts are specified for Right Control buttons only. You can now use your left Ctrl + (1-9) for other shortcuts :)
